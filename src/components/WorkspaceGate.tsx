...
export default function WorkspaceGate({ children }: { children: React.ReactNode }) {
  ...
  return <Page><PageContainer>{status === 'loading' ? <div role="status" className="space-y-3"><p>ログイン状態を確認中…</p><Skeleton className="h-6 w-2/3" /><Skeleton className="h-11 w-48" /></div> : <>
    <h1 className="text-2xl mb-4">ログインして練習を始める</h1>
    <p className="mb-6">単語リストと練習記録をアカウントに保存し、同じGitHubアカウントで別の端末からも利用できます。</p>
    <Button variant="default" onClick={() => signIn('github')}>GitHubでログインして続ける</Button>
  </>}</PageContainer></Page>
}
